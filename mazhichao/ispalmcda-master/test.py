...
parser.add_argument("--model_name", type=str, default="graphcodebert-base")
parser.add_argument("--model_name_or_path", type=str, default="output/graphcodebert-base/full_fine_tuning_seed_42/best_model")
parser.add_argument("--test_file", type=str, default="dataset/牛逼/test.csv")
...
# Load datasets
test_df = pd.read_csv(args.test_file, encoding='ISO-8859-1')


train_df = pd.read_csv("dataset/牛逼/train.csv", encoding='ISO-8859-1')
num_labels = len(train_df["label"].unique())
print(f" Using num_labels = {num_labels} from training set")

def clean_text(x):
    if pd.isna(x):
        return ""
    if isinstance(x, (list, dict, tuple, float, int)):
        return str(x).strip()
    return str(x).strip()
...
assert test_df["label"].min() >= 0, "Label contains negative values!"
assert test_df["label"].max() < num_labels, f"Test label {test_df['label'].max()} >= num_labels ({num_labels})!"
...
if not args.peft:
    ...
    tokenizer_path = f"./LLMs/{model_name}"
