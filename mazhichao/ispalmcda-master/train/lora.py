...
parser.add_argument("--model_name_or_path", type=str, default="./LLMs/roberta-base")
...
log_file_path = os.path.join(args.output_dir, model_name, f"lora_seed_{args.seed}", f"train_max_samples_{args.max_train_samples}_log.txt")
...
peft_config = LoraConfig(
    ...
    target_modules = LORA_IA3_TARGET_MODULES[model_name]["target_modules_lora"]
)
...
def collate_fn(examples):
    outputs = tokenizer.pad(examples, return_tensors="pt", padding=True, max_length=args.max_seq_length)
    return outputs
...
logger.info(f"LoRA-Trainable parameters: {model.get_nb_trainable_parameters()}")
...
for epoch in range(args.num_epochs):
    ...
    if avg_validation_loss < best_validation_loss:
        ...
        best_model_path = os.path.join(args.output_dir, model_name, f"lora_seed_{args.seed}", "best_model")
    ...
    save_path = os.path.join(args.output_dir, model_name, f"lora_seed_{args.seed}", f"epoch_{epoch + 1}")
...
with open(f"{args.output_dir}/{model_name}/training_time.txt", "a") as f:
    f.write(f"epoch: {args.num_epochs} lora: {str(training_time)}")
